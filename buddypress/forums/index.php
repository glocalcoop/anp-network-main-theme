...
<div id="buddypress">
	...
	<form action="" method="post" id="forums-directory-form" class="dir-form">
		...
		<div class="item-list-tabs" role="navigation">
			<ul>
				<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( __( 'All Topics %s', 'anp-network-main' ), '<span>' . bp_get_forum_topic_count() . '</span>' ); ?></a></li>

				<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

					<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( __( 'My Topics %s', 'anp-network-main' ), '<span>' . bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) . '</span>' ); ?></a></li>
		...
		<div class="item-list-tabs" id="subnav" role="navigation">
			<ul>
				...
				<li id="forums-order-select" class="last filter">
					...
					<label for="forums-order-by"><?php _e( 'Order By:', 'anp-network-main' ); ?></label>
					<select id="forums-order-by">
						<option value="active"><?php _e( 'Last Active', 'anp-network-main' ); ?></option>
						<option value="popular"><?php _e( 'Most Posts', 'anp-network-main' ); ?></option>
						<option value="unreplied"><?php _e( 'Unreplied', 'anp-network-main' ); ?></option>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form action="" method="post" id="forum-topic-form" class="standard-form">
					...
					<a name="post-new"></a>
					<h5><?php _e( 'Create New Topic:', 'anp-network-main' ); ?></h5>
					...
					<label for="topic_title"><?php _e( 'Title:', 'anp-network-main' ); ?></label>
					<input type="text" name="topic_title" id="topic_title" value="" maxlength="100" />

					<label for="topic_text"><?php _e( 'Content:', 'anp-network-main' ); ?></label>
					<textarea name="topic_text" id="topic_text"></textarea>

					<label for="topic_tags"><?php _e( 'Tags (comma separated):', 'anp-network-main' ); ?></label>
					<input type="text" name="topic_tags" id="topic_tags" value="" />

					<label for="topic_group_id"><?php _e( 'Post In Group Forum:', 'anp-network-main' ); ?></label>
					<select id="topic_group_id" name="topic_group_id">

						<option value=""><?php /* translators: no option picked in select box */ _e( '----', 'anp-network-main' ); ?></option>
					...
					<div class="submit">
						<input type="submit" name="submit_topic" id="submit" value="<?php esc_attr_e( 'Post Topic', 'anp-network-main' ); ?>" />
						<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php esc_attr_e( 'Cancel', 'anp-network-main' ); ?>" />
					</div>
			...
			<?php elseif ( bp_is_active( 'groups' ) ) : ?>
				...
				<div id="message" class="info">
					...
					<p><?php printf( __( "You are not a member of any groups so you don't have any group forums you can post in. To start posting, first find a group that matches the topic subject you'd like to start. If this group does not exist, why not <a href='%s'>create a new group</a>? Once you have joined or created the group you can post your topic in that group's forum.", 'anp-network-main' ), trailingslashit( bp_get_groups_directory_permalink() . 'create' ) ); ?></p>
